...
- [x] indexOf
- [x] splice
  - splice는 파괴적인 함수이다. 원본 배열을 변화시킨다.
  - splice(i)
...
- [x] slice
  - slice는 splice와 반대로 비파괴적인 함수이다. 원본 배열을 변화시키지 않는다.
  - splice(2, 4);
    - 배열의 2번째 인덱스부터 3번째 인덱스까지 뽑아내 새로운 배열을 만든다. 
- [x] reverse
  - array.reverse();
    - 배열을 뒤집는다. 파괴적인 함수이기 때문에 원본 배열을 변화시킨다.
    
- [x] find
  - 콜백함수는 boolean 값으로 return하고 true가 한 개라도 나오는 순간 해당 원소를 반환하고 순회를 종료한다.
  
- [x] filter
  - 원하는 조건을 찾는 순간 순회를 끝내지 않고 모든 원소를 한 바퀴 돈다.
  - 조건에 만족하는 원소들을 배열로 리턴한다.

- [x] map
  - 배열을 순회하면서 원소 각각에 원하는 조작을 하여 새로운 배열을 리턴한다.

- [x] some
  - 배열에 있는 원소들 중에서 조건에 하나라도 만족하면 원소가 존재하면 true를 리턴한다.
  
- [x] every
  - 배열에 있는 원소들 모두가 조건에 만족하면 true, 하나라도 만족 못할시 false를 리턴한다.
  
- [x] reduce
  - 순회할 때 이전에 순회했던 값을 다음 순회할 때 활용할 수 있다.
